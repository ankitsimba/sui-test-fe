        ...
        <div className="flex items-start justify-start w-full">
          <>
            <div className="w-8 mt-1">
              ...
              >
                {(record?.companyName && getInitials(record?.companyName)) || 'n/a'}
              </Avatar>
            </div>
            <div className="ml-2">
              <div className="font-bold  capitalize " title={record?.companyName}>
                {record?.companyName}
              </div>
      ...
      <Page
        ...
        primaryAction={
          <Button
            ...
            id="open"
          >
            Add Company <ArrowRightOutlined />
          </Button>
        }
      >
        <div className="bg-white ">
          <div className="px-5 py-4 flex gap-2">
            ...
            />
            <UploadCsvModal
              type="truckingCompany/importTruckCompaninesCSVData"
              modalTitle={'Import Companies'}
              apiMessage="Csv successfully updated"
              refetch={getAllTruckingCompany}
            />
          </div>
          <Divider />
          ...
          <div>
            <Table
              ...
              dataSource={truckingCompanyList?.records}
              scroll={{ x: 1000 }}
              columns={columns}
              locale={{
                emptyText: (
                  <div className="text-center flex justify-center items-center py-10">
                    <div>
                      <p className="text-lg">No company found!</p>
                      <img
                        className=""
                        src={SearchNotFound}
              ...
              footer={() => (
                <CheckValidation show={truckingCompanyList?.count > 5}>
                  <Row className="mt-2" type="flex" justify="end">
                    <Pagination
                      ...
                      current={currentPage}
                      pageSize={viewSize}
                      total={truckingCompanyList?.count}
        ...
        <Modal
          title={
            <p className="font-bold m-0">
              {isModalVisible === 'add' ? 'Add' : 'Update'} trucking company
            </p>
          }
            ...
            <Form
              ...
              onFinish={(values) => {
                ...
                if (isModalVisible === 'add') {
                  return dispatch({
                    type: 'truckingCompany/createTruckingCompany',
                    payload: { body },
                  })
                    .then((res) => {
                      if (res?._id) {
                ...
                return dispatch({
                  type: 'truckingCompany/updateTruckingCompany',
                  payload: {
                    body,
                    pathParams: {
                      truckCompanyId: isModalVisible,
                    },
                  },
                })
                  .then((res) => {
            ...
            >
              <Row gutter={24}>
                <Col xl={24} lg={24} md={24} sm={24} xs={24}>
                  ...
                  >
                    <Input size="large" autoFocus placeholder="Enter company name" />
                  </Form.Item>
                </Col>
...
export default connect(({ loading, truckingCompany, ports }) => ({
  updateLoading: loading?.effects['truckingCompany/updateTruckingCompany'],
  addLoading: loading?.effects['truckingCompany/createTruckingCompany'],
  loading: loading?.effects['truckingCompany/getAllTruckingCompany'],
