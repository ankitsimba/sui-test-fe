...
const Model = {
  ...
  effects: {
    ...
    *getEventDetail(_, { call, put }) {
      let apiResponse;
      let err;
      try {
        apiResponse = yield call(getEventDetail);
        yield put({
          type: 'setStates',
          payload: apiResponse?.event,
          key: 'eventDetail',
        });
      } catch (error) {
        // extract the error response from the server
    ...
    *getPartners({ payload }, { call, put }) {
      let apiResponse;
      let err;
      try {
        apiResponse = yield call(getPartners, payload || {});
        yield put({
          type: 'setStates',
          payload: apiResponse,
          key: 'partners',
